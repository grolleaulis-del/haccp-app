...
// Sélection employé (désormais derrière authentification)
...
// Scan étiquette (workflow rapide avec caméra auto) -- PUBLIC POUR TEST
...
Route::post('/lots/destroy-multiple', [ScanEtiquetteController::class, 'destroyMultiple'])->name('lots.destroy.multiple');
    ...
    Route::middleware(['auth'])->group(function () {
        ...
        // Familles (gestion des catégories produit)
        Route::resource('familles', FamilleController::class)->only(['index', 'store', 'update', 'destroy']);
        ...
        Route::get('/nettoyage/historique', [NettoyageController::class, 'historique'])->name('nettoyage.historique');
...
// Route DEMO pour validation design (Temporaire)
Route::get('/produits/demo', function() {
    return view('produits.demo');
})->name('produits.demo');

// Route toggle visibility (AJAX/Post)
Route::post('/produits/toggle-visibility', [ProduitController::class, 'toggleVisibility'])->name('produits.toggle-visibility');
